...
class Baserow
{
    ...
    private function execute_with_retry($handle, int $max_retries = 3): string|bool
    {
        for ($attempt = 0; $attempt <= $max_retries; $attempt++) {
            ...
            if ($attempt < $max_retries) {
                ...
                _l("Retry attempt %d for Baserow API (waiting %ds).", $attempt + 1, $wait_time);
    ...
    public function get(array $data = []): array
    {
        ...
        // make request with retry logic and return empty array on error
    ...
    public function post(array $data): Post_Result
    {
        ...
        // make request with retry logic and output on error
        $json = $this->execute_with_retry($handle);
        if (!is_string($json)) {
            _l(curl_error($handle));
            curl_close($handle);
